...
function get_debate_title($parent_id,$this_stand,$this_id){
	...
	if($parent_id!="0"){
		...
		if($parent_stand!=$this_stand){
			if($parent_stand=="1"){
				$reply_title = "<br /><a class=\"replay-title\" href=\"#comment-".$parent_id."\" title=\"".$parent_id."\"> (反驳：正方".$debate['dabete_obverse'][$parent_id]."辩 ".$parent_author.")</a>";
			}elseif($parent_stand=="0"){
				$reply_title = "<br /><a class=\"replay-title\" href=\"#comment-".$parent_id."\" title=\"".$parent_id."\"> (反驳：反方".$debate['dabete_reverse'][$parent_id]."辩 ".$parent_author.")</a>";
			}else{
				$reply_title = "<br /><a class=\"replay-title\" href=\"#comment-".$parent_id."\" title=\"".$parent_id."\"> (反驳：中立".$debate['dabete_other'][$parent_id]."辩 ".$parent_author.")</a>";
			}
		}else{
			if($parent_stand=="1"){
			...
			$reply_title = "<a class=\"replay-title\" href=\"#comment-".$parent_id."\" title=\"".$parent_id."\"> (回复".$reply_name."辩 ".$parent_author.")</a>";
		}
	}
	if($this_stand=="1")
		$title="正方".$debate['dabete_obverse'][$this_id]."辩 ".get_comment_author_link().$reply_title;
	elseif($this_stand=="0")
		$title="反方".$debate['dabete_reverse'][$this_id]."辩 ".get_comment_author_link().$reply_title;
	else 
		$title="中立".$debate['dabete_other'][$this_id]."辩 ".get_comment_author_link().$reply_title;
	return $title;
}
