...
function custom_logo() {
	...
	if( $custom_logo != '' ){
		...
		#wphead { background: url('.BMURL.'/images/'.$custom_logo.') no-repeat center bottom  !important; }
		#header-logo { background-image: url('.BMURL.'/images/'.$admin_logo.') !important; }
		</style>';
	}
}
function custom_login_logo() {
	global $settings;
	$custom_login_logo = $settings['custom_login_logo'];
	if( $custom_login_logo != '' ){
	  echo '<style type="text/css">
		h1 a { background-image:url('.BMURL.'/images/'.$custom_login_logo.') !important; }
		</style>';
	}
}
...
function bm_admin_head() {  
	switch ($_GET['page']){
		case "bm" :
		case "bm_uploads" :
			print '<link rel="stylesheet" href="'.BMURL.'/css/admin-style.css" type="text/css" media="screen" />';
